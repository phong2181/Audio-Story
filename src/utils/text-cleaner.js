...
import { restoreDiacritics } from './vietnamese-diacritics.js';
...
function isLikelyVietnameseText(text) {
    ...
    const vietnameseChars = /[àáảãạăằắẳẵặâầấẩẫậèéẻẽẹêềếểễệìíỉĩịòóỏõọôồốổỗộơờớởỡợùúủũụưỳýỷỹỵđĐ]/;
    if (vietnameseChars.test(text)) return true;

    const words = text.split(/\s+/).filter(Boolean);
    if (words.length === 0) return false;

    const vietnameseLikeWords = words.filter((word) => {
        const normalized = word.toLowerCase();
        return normalized.length >= 2 && /[aeiouy]/.test(normalized) && !/[fwzj]/.test(normalized) && !/[^a-zà-ỹ]/i.test(normalized);
    });

    return vietnameseLikeWords.length >= Math.max(1, Math.floor(words.length * 0.6));
}
...
export function cleanTextForTTS(text) {
    ...
    const cleanedText = text.replace(emojiRegex, '')
        ...
        .replace(/\s—/g, '.')
        .replace(/\b_\b/g, ' ')
        // SỬA Ở ĐÂY: Cho phép giữ lại dấu gạch nối giữa các từ/âm tiết (ví dụ do transliterator tạo ra)
        // Thay vì xóa sạch mọi dấu trừ, ta chỉ lọc các dấu trừ đơn lẻ không cần thiết.
        // Hoặc đơn giản là comment dòng xóa dash đi nếu bạn muốn giữ lại format gạch nối:
        // .replace(/(?<!\d)-(?!\d)/g, ' ') -> XÓA HOẶC ĐỔI THÀNH GIỮ NGUYÊN DẤU -
        // FIX: Added \u0250-\u02AF (IPA Extensions range) to preserve IPA phoneme characters
        // like ɗ, ɛ, ɔ, ɤ, ɯ, ʃ, ʈ, ʂ, θ, ŋ, ɲ which are critical for Vietnamese pronunciation
        .replace(/[^\u0000-\u024F\u0250-\u02AF\u1E00-\u1EFF\-]/g, '');
...
export async function processTextForTTS(text) {
    ...
    // Step 1.5: Restore Vietnamese diacritics if configured (optional)
    // This restores diacritics for text without marks (e.g., "toi di hoc" -> "tôi đi học")
    const textWithDiacritics = (config.enableDiacriticsRestoration !== false)
        ? restoreDiacritics(cleanedText)
        : cleanedText;
    if (isDebugEnabled(config)) {
        debugLog(config, 'Step 1.5: Diacritics Restoration', {
            before: cleanedText,
            after: textWithDiacritics,
            enabled: config.enableDiacriticsRestoration !== false
        });
    }

    // Then, process Vietnamese text (convert numbers, dates, times, etc.)
    const vietnameseProcessedText = processVietnameseText(textWithDiacritics, config);
    if (isDebugEnabled(config)) {
    ...
    if (config.enableTransliteration && !isLikelyVietnameseText(textAfterWordReplacement)) {
    ...
    // IMPORTANT: Keep dashes from transliteration (e.g. "xin-ga-po", "công-tê-nơ")
    // The hyphen connects syllables of transliterated non-Vietnamese words.
    // These are later split into separate phoneme chunks by createNaturalPiperChunks()
    // DO NOT replace dashes with spaces as it would merge syllables into gibberish.
    // processedText = processedText.replace(/-+/g, ' '); // <-- REMOVED this destructive step
